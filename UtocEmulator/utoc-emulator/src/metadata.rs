...
use retoc::ser::{ReadExt, Readable};
...
impl UtocMetadata {
    ...
    pub(crate) fn add_from_utocmeta(&mut self, data: &[u8], engine: EngineVersion) -> GenericResult<()> {
        let mut reader = Cursor::new(data);
        // read header
        let version: UtocMetaVersion = reader.de()?;
        let alt_auto_import_count: u32 = reader.de()?;
        let manual_v1_count: u32 = reader.de()?;
        let compressed_package_count: u32 = reader.de()?;
    ...
    pub(crate) fn add_from_uassetmeta(&mut self, key: FPackageId, path: &Path) -> GenericResult<()> {
